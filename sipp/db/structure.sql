...
CREATE  TABLE IF NOT EXISTS `member` (
  ...
  `member_nip` VARCHAR(255) NOT NULL ,
  `username` VARCHAR(45) NULL ,
  `password` VARCHAR(45) NULL ,
  `member_full_name` VARCHAR(255) NULL ,
  `member_sex` ENUM('MALE', 'FEMALE') NULL ,
  `member_birth_place` VARCHAR(45) NULL ,
  `member_birth_date` DATE NULL ,
  `member_school` VARCHAR(255) NULL ,
  `member_phone` VARCHAR(45) NULL ,
  `member_address` TEXT NULL ,
  `member_image` VARCHAR(100) NULL ,
  `member_mentor` VARCHAR(255) NULL ,
  `member_division` VARCHAR(255) NULL ,
  `member_status` TINYINT(1) NULL DEFAULT 0 COMMENT '0=\'Non-Active\', 1=\'Active\'' ,
  `member_input_date` TIMESTAMP NULL ,
  `member_last_update` VARCHAR(45) NULL ,
  PRIMARY KEY (`member_id`, `member_nip`) )
ENGINE = InnoDB;
...
CREATE  TABLE IF NOT EXISTS `present` (
  ...
  `present_desc` ENUM('Hadir', 'Sakit', 'Izin', 'Alfa') NULL ,
  `present_is_late` TINYINT(1) NULL DEFAULT 0 ,
  `present_is_before` TINYINT(1) NULL DEFAULT 0 ,
  `member_member_id` INT NULL ,
  `member_member_nip` VARCHAR(255) NULL ,
  PRIMARY KEY (`present_id`) ,
  INDEX `fk_present_member1_idx` (`member_member_id` ASC, `member_member_nip` ASC) ,
