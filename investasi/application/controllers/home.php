...
class home extends CI_Controller {
    ...
    function getchart($type = '') {
        ...
        $table = "";
        $tablee = "";
        if ($type == 'tbl_pertumbuhan_ekonomi2') {
            $tablee = "tbl_pertumbuhan_ekonomi";
        } else {
            $tablee = $type;
        }
        ...
        if ($this->input->post('menu_id') == '42') {
            $where .=" AND flag='D' ";
        } if ($this->input->post('menu_id') == '32') {
            $where .=" AND flag='L' ";
        }
    ...
    function nyimpenposting() {
        ...
        foreach ($_POST as $k => $v)
            $post[$k] = $this->input->post($k, TRUE, TRUE);
    ...
    function news($offset = NULL) {
        ...
        $data['headline'] = $this->getdisplay('headline');
        $data['berita'] = $this->getdisplay('berita');
        $data['isi'] = $this->db->get_where('tbl_berita', null, 10, $offset)->result_array();
        $config['per_page'] = 10;
        $config['uri_segment'] = 3;
        $config['base_url'] = site_url('berita/index');
        $config['total_rows'] = count($this->db->get(array('tbl_berita'))->result_array());
        $this->pagination->initialize($config);

        $data['content'] = 'berita_indeks';
        $data['type'] = 'beranda';
        $this->load->view('index', $data);
    }
